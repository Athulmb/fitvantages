...
import { SPORTS_API, BASE_URL } from "../config"; // import API
...
const SportsCard = ({ sport, onClick }) => {
  return (
    ...
    >
      ...
      <img
        src={sport.image.startsWith("http") ? sport.image : `${BASE_URL}${sport.image}`}
        alt={sport.title}
...
const SportsFormat = () => {
  ...
  useEffect(() => {
    const fetchSports = async () => {
      try {
        const res = await axios.get(SPORTS_API);
        if (res.data.success) {
          setSports(res.data.sports || []); // adapt if response is different
        }
      } catch (error) {
  ...
  if (loading) {
    return (
      <section className="w-full py-12 text-center text-white">
        Loading sports...
      </section>
    );
  }
