...
export default function FitnessSignup() {
    // Simulated navigation function for demo
    const navigate = (path) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Store token if you want to log user in immediately (optional)
                // if (data.token) {
                ...
                setTimeout(() => {
                    // You can redirect to dashboard if token is stored, or to signin
                ...
                if (data.errors && Array.isArray(data.errors)) {
                    ...
                    const missingFieldNames = Object.keys(data.missingFields)
                        .filter(field => data.missingFields[field])
        ...
        <div className="min-h-screen flex bg-black">
            ...
            <div className="w-full lg:w-1/2 bg-transparent flex items-center justify-center p-6 lg:p-12">
                <div className="w-full max-w-md">
                    <h1 className="text-3xl lg:text-4xl font-bold text-white text-center mb-8">Create Account</h1>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="relative group rounded-xl overflow-hidden">
                            <div className={`absolute inset-0 bg-white/10 rounded-xl backdrop-blur-md border transition-all duration-300 ${
                                getFieldError('fullName') ? 'border-red-500/50' : 'border-white/20 group-hover:border-[#7FFA88]'
                            }`}></div>
                            <div className="relative z-10">
                                <input
                                    ...
                                    className="w-full bg-transparent rounded-lg px-4 py-3 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#7FFA88] transition-all duration-200"
                                    disabled={loading}
                                    required
                                />
                            </div>
                        ...
                        <div className="relative group rounded-xl overflow-hidden">
                            <div className={`absolute inset-0 bg-white/10 rounded-xl backdrop-blur-md border transition-all duration-300 ${
                                getFieldError('email') ? 'border-red-500/50' : 'border-white/20 group-hover:border-[#7FFA88]'
                            }`}></div>
                            <div className="relative z-10">
                                <input
                                    type="email"
                            ...
                            {getFieldError('email') && (
                                <p className="text-red-400 text-xs mt-1 ml-1">{getFieldError('email')}</p>
                            )}
                        </div>

                        {/* Password + Confirm */}
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {[{ label: 'Password', name: 'password' }, { label: 'Confirm Password', name: 'confirmPassword' }].map(
                                (field) => (
                                    <div key={field.name}>
                                        <div className="relative group rounded-xl overflow-hidden">
                                            <div className={`absolute inset-0 bg-white/10 rounded-xl backdrop-blur-md border transition-all duration-300 ${
                                                getFieldError(field.name) ? 'border-red-500/50' : 'border-white/20 group-hover:border-[#7FFA88]'
                                            }`}></div>
                                            <div className="relative z-10">
                                                <input
                                                    type="password"
                                        ...
                                        {getFieldError(field.name) && (
                                            <p className="text-red-400 text-xs mt-1 ml-1">{getFieldError(field.name)}</p>
                                        )}
                                    </div>
                                )
                            )}
                        ...
                        <div className="flex items-start gap-3 mt-6">
                            <input
                                ...
                                onChange={handleInputChange}
                                className="mt-1 w-4 h-4 text-[#7FFA88] bg-gray-800 border-gray-600 rounded focus:ring-[#7FFA88] focus:ring-2"
                                disabled={loading}
                                required
                            />
                            <span className="text-gray-300 text-sm">
                                By creating your account, you agree to our{' '}
                                <span className="text-[#7FFA88] hover:text-green-300 underline cursor-pointer">
                                    Terms and Conditions
                                </span>
                            </span>
                        </div>
                        {getFieldError('agreeToTerms') && (
                            <p className="text-red-400 text-xs ml-7">{getFieldError('agreeToTerms')}</p>
                        )}
                        ...
                        <button
                            ...
                            className={`w-full font-semibold py-3 px-4 rounded-lg transition-all duration-200 transform ${
                                loading
                                    ...
                                    : 'bg-[#7FFA88] hover:bg-[#6ef67d] text-gray-900 hover:scale-[1.02] hover:shadow-[0_0_15px_4px_#7FFA88]'
                            }`}
                        >
                            {loading ? (
                    ...
                    <div className="text-center mt-6">
                        ...
                        <button
                            ...
                            disabled={loading}
                            className={`w-full border py-3 px-4 rounded-lg transition-all duration-300 ${
                                loading
                                    ? 'border-gray-600 text-gray-500 cursor-not-allowed'
                                    : 'bg-transparent border-gray-300 text-white hover:border-green-400 hover:shadow-[8px_8px_20px_rgba(74,222,128,0.5)]'
                            }`}
                        >
                            Sign In
                        </button>
